...
## Session times (WhatsApp Event message)

When a winner is announced, the bot posts it as a native WhatsApp **Event** (with a clickable date) instead of a plain text message — when it can figure out the slot's start time. Hebrew labels of the form `<day> <time-of-day>` (e.g. `שלישי ערב`) are resolved automatically; otherwise the slot falls back to a plain text announcement.

To override the defaults or add non-conforming labels, add a `sessionTimes` block to `config.json` (optional):
...
"sessionTimes": {
  ...
  "slotTimes": {
    "שבת ערב": { "dayOfWeek": "saturday", "time": "18:00" }
  }
}
```
