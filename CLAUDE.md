...
All jobs are idempotent (check DB state before acting) and are passed a shared `ctx = { config, db, whatsapp, googleForm }`. The scheduler wrapper (`wrap` in `src/scheduler.js`) retries a failed job up to 2 times, 10 minutes apart — safe precisely because jobs are idempotent. Beyond that, recovery relies on idempotency at the next scheduled run, or on the WhatsApp wrapper's in-flight retry (below).

Each job also has a **warmup cron 15 minutes earlier** (08:15 Sun/Tue/Wed, 19:45 Wed — the latter only when a tiebreaker poll exists): `whatsapp.warmup()` probes `getState()` twice and forces a client reinit if the browser is dead, so the ~1–2 min reinit cost is paid before the job fires and the message goes out immediately at job time.
...
Every public op is wrapped with `withTransientRetry`: on a Puppeteer-level transient error (`detached Frame`, `Execution context was destroyed`, `Target closed`, `Session closed`, `Protocol error`, `Most likely the page has been closed`) the wrapper forces a full client re-init and retries the op once. This catches the case where the underlying Chromium frame detaches silently after multi-day uptime without firing whatsapp-web.js's `disconnected` event. `src/index.js` also runs a liveness probe every 5 minutes (`client.getState()`, 30s-bounded): a probe that throws or resolves to a non-`CONNECTED` state counts as a failure, and 4 consecutive failures trigger the same re-init — so detachment is usually healed within ~20 min, long before the next cron fires. The threshold exists because a single `getState()` hiccup is common and every reinit gambles a little with the session.

Send ops (`sendText`/`sendPoll`/`sendEvent`) are bounded at 120s via `withTimeout`. A send timeout deliberately does **not** match the transient patterns (no reinit-and-resend — if the send actually landed, a resend would duplicate the message); the scheduler-level job retry handles recovery instead. Teardown is hardened: `client.destroy()` is bounded at 60s and the Chrome process is SIGKILLed if it doesn't exit, and stale `Singleton*` lock files in `.wwebjs_auth/session/` are cleared before each launch — a leaked browser or stale lock is what used to turn one bad reinit into a storm that eventually lost the auth.
...
- **Browser is `google-chrome-stable`, NOT snap chromium.** Installed via the official Google apt repo at `/usr/bin/google-chrome-stable`. `findChromePath()` in `src/whatsapp.js` checks for it first. Confirm at startup: the log line `Puppeteer: using system Chrome at /usr/bin/google-chrome-stable` must appear. If it ever falls back to `/usr/bin/chromium-browser`, the WhatsApp session will appear to vanish on each restart (see historical bullet below) — reinstall chrome stable.
- **WhatsApp linked-device session lives at `/opt/dnd-poll-bot/.wwebjs_auth/session/Default/IndexedDB/https_web.whatsapp.com_0.indexeddb.leveldb/`.** That's the real `LocalAuth` path, and it persists across bot restarts and `withTransientRetry` reinits. To force a fresh QR scan: `rm -rf /opt/dnd-poll-bot/.wwebjs_auth` while the bot is stopped, then start in tmux and scan.
- **Singleton lock cleanup.** If chrome stable exits abruptly it can leave `Singleton{Lock,Cookie,Socket}` lock files behind, and new launches can hang. The bot now auto-clears them from the LocalAuth profile (`.wwebjs_auth/session/`) before every client launch. If a launch still hangs, also check the default profile dir: `rm -f /root/.config/google-chrome/Singleton*`.
- **(Historical, pre-2026-05-17 — no longer applies.)** The bot used to run on `/usr/bin/chromium-browser` (a snap wrapper). Snap confinement caused chromium to IGNORE puppeteer's `--user-data-dir` and write its profile to `/root/snap/chromium/common/chromium/Default/` instead. The WhatsApp session lived there, not in `.wwebjs_auth/`. That's why old debug notes reference the snap path; on dirty exits IndexedDB sometimes failed to flush, silently corrupting the session. Migrated off snap → chrome-stable on 2026-05-17. Snap chromium remains installed but unused.
- **`waitForReady` default is 900s** (`src/whatsapp.js`). First-time / re-link QR auth needs a generous human-paced window (admin may not be watching the log when the session expires), and reinit on a slow VPS can take ~2 min for chromium to come back up. Don't drop this below ~180s without a reason.
