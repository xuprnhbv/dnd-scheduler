...
All jobs are idempotent (check DB state before acting) and are passed a shared `ctx = { config, db, whatsapp, googleForm }`. The scheduler wrapper only logs errors — it does not retry; jobs that need recovery must rely on idempotency at the next scheduled run, or on the WhatsApp wrapper's in-flight retry (below).
...
- **Singleton lock cleanup.** If chrome stable exits abruptly it can leave `Singleton{Lock,Cookie,Socket}` in `/root/.config/google-chrome/`. New launches can hang. Clear them: `rm -f /root/.config/google-chrome/Singleton*`.
